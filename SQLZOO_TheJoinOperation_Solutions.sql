-- The JOIN Operation solutions
-- https://sqlzoo.net/wiki/The_JOIN_operation

/*
1) The first example shows the goal scored by a player with the last
name 'Bender'. The * says to list all the columns in the table - a
...
goal in game 1012. Now we want to know what teams were playing in
that match.
...
the game table. The ON says how to figure out which rows in game go
with which rows in goal - the id from goal must match matchid from
game. (If we wanted to be more clear/specific we could say ON
...
Modify it to show the player, teamid, stadium and mdate and for
every German goal.
*/
...
game JOIN eteam ON (team1=eteam.id) or game JOIN eteam ON
(team2=eteam.id)
...
*/
SELECT teamname, COUNT(*) AS 'goals scored'
FROM eteam
...
*/
SELECT stadium, count(*) AS 'goals scored'
FROM game
JOIN goal ON game.id = goal.matchid
GROUP BY stadium;
...
exercises.
*/
SELECT mdate, team1, SUM(CASE WHEN goal.teamid = game.team1 THEN 1
  ELSE 0 END) AS score1, team2, SUM(CASE WHEN goal.teamid = game.team2

...
*/
SELECT stop, s.name
FROM route AS r, stops AS s
WHERE num = 4 AND company = 'LRT' AND s.id = r.stop;
...
rather than by number. Change the query so that the services
between 'Craiglockhart' and 'London Road' are shown. If you are tired
...
to Sighthill.
Show the bus no. and company for the first bus, the name of the

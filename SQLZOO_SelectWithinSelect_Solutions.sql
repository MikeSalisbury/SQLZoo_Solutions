-- Solutions for Select Within Select tutorial.
...
*/
SELECT name, ROUND(100 * (population/(SELECT population
...
FROM world AS x
WHERE name = (SELECT name FROM world AS y WHERE
  x.continent = y.continent ORDER BY name LIMIT 1);
